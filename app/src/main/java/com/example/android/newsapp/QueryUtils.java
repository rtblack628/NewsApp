...
public final class QueryUtils {
    ...
    /**
     * Query The Guardian's dataset and return a list of {@link News} objects.
     */
    public static List<News> fetchNewsData(String requestUrl, Context context) {
        ...
        List<News> news = extractNewsFromJson(jsonResponse, context);
     ...
     */
    private static List<News> extractNewsFromJson(String newsJSON, Context context) {
        ...
        try {
            ...
            for (int i = 0; i < resultsJSONArray.length(); i++) {
                ...
                JSONObject currentFieldsObject = currentNews.getJSONObject("fields");

                // Extract the value for the key called "headline"
                String headline = currentFieldsObject.getString("headline");

                // Extract the value for the key called "byline"
                String byline = currentFieldsObject.getString("byline");

                // Extract the value for the key called "publication"
                String publication = currentFieldsObject.getString("publication");

                // Extract the value for the key called "trailText"
                String trailText = currentFieldsObject.getString("trailText");

                // Create a new {@link News} object with the headline, byline, publication,
                // trailText and url from the JSON response. Add the new {@link News} to
                // the list of news items.
                news.add(new News(headline, byline, publication, trailText, url));
            }
